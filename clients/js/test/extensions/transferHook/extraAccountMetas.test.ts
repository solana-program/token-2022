...
function validateStateAccountData(extraAccounts: Uint8Array[]): Uint8Array {
    return new Uint8Array([
        ...new Uint8Array(12), // u64 instructionDiscriminator + u32 length (both skipped when decoding)
...
describe('resolveSeeds', () => {
    ...
    it('resolves an account-data seed by fetching the account over rpc', async () => {
        const { rpc, svm } = await createTestClient();
...
describe('resolveExtraAccountMeta', () => {
    it('resolves a literal-pubkey (discriminator 0) meta', async () => {
        const { rpc } = await createTestClient();
    ...
    it('resolves a pubkey-data (discriminator 2) meta', async () => {
        const { rpc } = await createTestClient();
        const addressBytes = getAddressEncoder().encode(mint);
        const instructionData = new Uint8Array([0xff, ...addressBytes]);
    ...
    it('derives a PDA off the transfer hook program (discriminator 1)', async () => {
        const { rpc } = await createTestClient();
    ...
    it('throws when the account-index discriminator is out of bounds', async () => {
        const { rpc } = await createTestClient();
